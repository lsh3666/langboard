...
if [ -n "${POSTGRES_EXTERNAL_MAIN_URL}" ] || [ -n "${POSTGRES_EXTERNAL_REPLICA_URL}" ]; then
    ...
    OUTPUT=$(pg_dump --no-sync --inserts --clean --no-owner -b -v -f "$FILE_PATH_PREFIX.dump" "${PG_EXTERNAL_URL}" 2>&1)
else
    record_log "Internal PostgreSQL mode detected. Using replica host."
    OUTPUT=$(PGPASSWORD="${PG_PASS}" pg_dump --no-sync -h "${PG_MASTER_HOST}" -U "${PG_USER}" -p "${PG_MASTER_PORT}" --inserts --clean --no-owner -b -v -f "$FILE_PATH_PREFIX.dump" "${PG_DB}" 2>&1)
fi
DUMP_EXIT_CODE=$?
record_log "$OUTPUT"
...
if [ -n "${DB_BACKUP_UPLOAD_URL}" ]; then
    ...
    CURL_ARGS=(--silent --show-error --fail --location --request POST --form "file=@$FILE_PATH_PREFIX.tar.gz")
    HEADER_COUNT=0

    if [ -n "${DB_BACKUP_UPLOAD_HEADERS}" ]; then
        IFS='|' read -r -a HEADER_ITEMS <<< "${DB_BACKUP_UPLOAD_HEADERS}"
        for header in "${HEADER_ITEMS[@]}"; do
            header=$(echo "$header" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
            if [ -n "$header" ]; then
    ...
    UPLOAD_OUTPUT=$(curl "${CURL_ARGS[@]}" "${DB_BACKUP_UPLOAD_URL}" 2>&1)
    UPLOAD_EXIT_CODE=$?
