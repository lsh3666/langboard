...
if [ -z "$ROOT_TOKEN" ]; then
    echo "❌ Error: KEY_PROVIDER_OPENBAO_ROOT_TOKEN environment variable not set"
    echo "   Start OpenBao in dev mode:"
    echo "   bao server -dev -dev-root-token-id=\"test\""
    echo "   Then set in .env file:"
    echo "   KEY_PROVIDER_OPENBAO_ROOT_TOKEN=test"
    exit 1
fi

# Check if OpenBao is running
echo "🔍 Checking if OpenBao is running..."
HEALTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$VAULT_URL/v1/sys/health")
if [ "$HEALTH_STATUS" != "200" ] && [ "$HEALTH_STATUS" != "429" ]; then
    echo "❌ Error: OpenBao is not running at $VAULT_URL (status: $HEALTH_STATUS)"
    echo "   Start OpenBao in dev mode first"
    exit 1
fi
...
if [ -d "$RESTORE_DIR" ]; then
    ...
    if [ "$KEY_COUNT" -gt 0 ]; then
        ...
        # Iterate over each file in the restore directory
        for KEY_FILE in "$RESTORE_DIR"/*; do
            if [ -f "$KEY_FILE" ]; then
                ...
                if [ "$CHECK_STATUS" = "200" ]; then
                    echo "   ⏭️  Skipped (already exists): $KEY_ID"
                    SKIPPED=$((SKIPPED + 1))
                else
                    # Write key to vault
        ...
        echo "ℹ️  No key files found in $RESTORE_DIR"
        echo ""
    fi
else
    echo "ℹ️  No vault-data directory found (skipping restore)"
    echo ""
fi
...
echo "   - Vault Address: $VAULT_URL"
echo ""
echo "🚀 You can now use the application with OpenBao!"
