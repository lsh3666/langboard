...
class McpRoleFilter(BaseFilter, Generic[_TMethod]):
    def __init__(self):
        self._filtered: dict[_TMethod, tuple[type, list[str], _McpRoleFinderFunc]] = {}

    def add(
        self, role_model: type[_TRoleModel], actions: list[str | Enum], role_finder: _McpRoleFinderFunc
    ...
    def get_filtered(self, method: _TMethod) -> tuple[type[BaseRoleModel], list[str], _McpRoleFinderFunc]:
